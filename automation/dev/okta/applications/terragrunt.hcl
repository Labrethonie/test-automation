...
inputs = {
    ...
    hide_web = false
    pkce_required = false
    oauth_scopes = ["okta.apps.read", "okta.roles.read"]
    auto_submit_toolbar = false
    accessibility_self_service = false
    app_links_json = jsonencode({"workflow_login":true})
    user_name_template = "$${source.login}"
    user_name_template_type = "$${source.login}"
    users_assigned = ["jerome.heymonet@backmarket.com", "gregory.novosad.ext@backmarket.com", "remy.angel@backmarket.com"]
}